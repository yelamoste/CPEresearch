...
<body class="body bg-ghostwhite" style="overflow-x:hidden;">
    ...
    <?php
        ...
        $query = "SELECT mor_one.*, response.*
        FROM mor_one
        INNER JOIN response
        ON mor_one.idmor_one = response.idmor_one
        WHERE mor_one.adviser = '$userid' AND mor_one.idmor_one = '$proposalid'";
        
        // $query = "SELECT comment FROM response WHERE idmor_one = '$proposalid'";
        $result = mysqli_query($conn, $query);
        $countRow = mysqli_fetch_array($result, MYSQLI_ASSOC);

        if ($countRow) {
            $comment = $countRow['comment'];
            $adviser_response = $countRow['adviser_response'];
        }else{
            $comment = "No comment/suggestion";
        }
    ?>
                ...
                <div class="row box-Response mt-2">
                    ...
                    <div class="approvResponse px-0" style="font-size:14px;box-shadow: 0px 4px 4px 0px rgba(0,0,0,0.25);">
                        <table class="table bg-gray400" style="font-size: 14px;">
                            <thead class="border">
                                <tr>
                                    <th scope="col" class="col-9 ms-1">Research Adviser</th>
                                    <th scope="col">Status</th>
                                </tr>
                            <tbody class="border">
                                <tr>
                                    <td class="col-9 ms-1"><?php echo $_SESSION['username']; ?></td>
                                    <td>
                                        <div class="checkResponse row d-flex align-items-end">
                                            <div class="col form-check">
                                                <span class="text-uppercase badge bg-<?php echo $adviser_response; ?> text-ghostwhite p-1"><?php echo $adviser_response; ?></span>
                                            </div>
                                        </div>  
                                    </td>
                    ...
                    <div class="text-end p-0">
                        <a href="../faculty/viewApprovalsheet.php?id=<?php echo $_SESSION['proposalid']; ?>"><small>View Approval Sheet & Comment</small></a>
                    </div>
                </div>
